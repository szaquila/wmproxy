...
impl WMCore {
	...
	/// 如果不传在上级, 则构建中心服处理该请求
	async fn deal_center_stream<T>(&mut self, inbound: T, _addr: SocketAddr, tls_client: Option<Arc<rustls::ClientConfig>>) -> ProxyResult<()>
	where
	...
	/// 没有上级直接处理当前代理数据
	async fn deal_client_stream<T>(&mut self, inbound: T, addr: SocketAddr) -> ProxyResult<()>
	where
		T: AsyncRead + AsyncWrite + Unpin + Send + Sync + 'static,
	{
		let iface_var = std::env::var("WMPROXY_IFACE").unwrap_or_default();
		let iface = iface_var.as_str();
		if !iface.is_empty() {
			if let Ok(local) = iface.parse::<SocketAddr>() {
				if local.ip().to_string().eq(&addr.ip().to_string()) {
					log::warn!("deal_client_stream: {} ignore", addr);
					return Ok(());
				}
			} else if let Ok(local) = iface.parse::<Ipv4Addr>() {
				if local.to_string().eq(&addr.ip().to_string()) {
					log::warn!("deal_client_stream: {} ignore", addr);
					return Ok(());
				}
			}
		...
		if let Some(option) = &mut self.option.proxy {
			let proxy_server = ProxyServer::new(option.flag, option.username.clone(), option.password.clone(), option.udp_bind.clone(), None);
			tokio::spawn(async move {
				// tcp的连接被移动到该协程中，我们只要专注的处理该stream即可
				log::warn!("deal_client_stream: {}", addr);
	...
	async fn multi_tcp_listen_work(listens: &mut Vec<TcpListener>) -> (io::Result<(TcpStream, SocketAddr)>, usize) {
		if !listens.is_empty() {
			let (conn, index, _) = select_all(listens.iter_mut().map(|listener| Helper::tcp_accept(listener).boxed())).await;
			(conn, index)
		} else {
			let pend = std::future::pending();
			let () = pend.await;
	...
	async fn multi_udp_listen_work(listens: &mut Vec<StreamUdp>) -> (io::Result<(Vec<u8>, SocketAddr)>, usize) {
		if !listens.is_empty() {
			let (data, index, _) = select_all(listens.iter_mut().map(|listener| listener.next().boxed())).await;
			if data.is_none() {
	...
	pub async fn ready_serve(&mut self) -> ProxyResult<()> {
		if let Some(option) = &mut self.option.proxy {
			(self.proxy_accept, self.proxy_client, self.client_listener, self.center_listener, self.center_client) = option.bind().await?;
		}

		if let Some(option) = &mut self.option.proxy {
			(self.map_http_listener, self.map_https_listener, self.map_tcp_listener, self.map_proxy_listener, self.map_accept) = option.bind_map().await?;
		}

		self.http_servers = self.option.http.clone().unwrap_or(HttpConfig::new()).convert_server_config();

		self.stream_config = Some(Arc::new(Mutex::new(self.option.stream.clone().unwrap_or(StreamConfig::new()))));
	...
	pub async fn run_serve(&mut self, mut receiver_close: Receiver<()>, mut sender_close: Option<Sender<()>>) -> ProxyResult<()> {
