				...
				if !iface.is_empty() {
					if let Ok(local) = iface.parse::<SocketAddr>() {
						if let Err(e) = socket.bind(local) {
							log::trace!("本地{iface}绑定失败, 原因: {:?}", e);
						}
					} else if let Ok(_) = iface.parse::<IpAddr>() {
						if let Ok(local) = format!("{}:0", iface).parse::<SocketAddr>() {
							if let Err(e) = socket.bind(local) {
								log::trace!("本地{iface}绑定失败, 原因: {:?}", e);
							}
						}
					} else {
						let ifaces = iface.as_bytes();
						if let Err(e) = socket.bind_device(Some(ifaces)) {
							log::trace!("本地{iface}绑定失败, 原因: {:?}", e);
						}
					}
				}
